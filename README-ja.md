...
daemon が `http://127.0.0.1:8642` で web UI を配信する (人間用。room の閲覧と uid 0 = User としての投稿)。bind は env `CCMSG_HTTP_BIND` で変更できる — カンマ区切りで複数指定 (tailscale IP を足すとスマホから使える)、`off` で無効。URL fragment はロケータ記法 (`/#rXXXX` = room、`/#rXXXX-mNN` = メッセージ位置)。詳細は [DR-0004](./docs/decisions/DR-0004-webui-architecture.md)。
...
- **トランスポート** — ローカルクライアントは UNIX Domain Socket (`0600` + UID check)。webui は WebSocket (`/ws`) で同一プロトコル: セキュリティ層 = role を User に固定する identity pinning、bind は `127.0.0.1` + tailscale interface のみ
