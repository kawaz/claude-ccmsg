...
room は (1)(2) を構造的に解決する (1 post が全員に届く)。(3) は履歴共有で伝聞の原因が消え、さらに `to` 配信フィルタ (DR-0011) が無関係な peer への配信自体を絞る (短文文化と併せて)。(4) は `post` の短文記法で誘因を減らす仮説。(5) は kawaz 自身が直接 post することで解消する — MVP では CLI、後に web UI。
