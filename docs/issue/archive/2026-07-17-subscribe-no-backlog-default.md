...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-21T03:27:39+09:00
discard_reason:
pending_reason:
close_reason: ["done:実装済みを確認(close漏れ)", "done:protocol SubscribeRequest.backlog + RoomCursorsStreamEvent", "done:daemon は since/since_seq/backlog:true なしの room を ev:room_cursors に集約し backlog を送らない", "done:CLI は room_cursors を stdout drop、旧挙動は backlog:true(webui使用中) または --since で opt-in", "done:SessionStart hook / SKILL.md / --help-full も追従済み", "done:受け入れ条件5項目充足"]
blocked_by:
origin: 自リポ TODO
---
...
- [x] `subscribe` のデフォルトで過去 msg が再送されず、接続時イベントが
      `{roomid, last_mid}` 一覧のみになる
- [x] 各セッションが記憶している mid と比較し、進んでいれば `read` で追いつける
      経路が daemon 側に用意されている
- [x] 従来の全バックログ再送挙動が `--since` 等の明示オプションで引き続き利用可能
- [x] daemon の subscribe ハンドラ + CLI 両方に変更が反映されている
- [x] SKILL.md / `--help-full` の記述がこのデフォルト変更に追従している
