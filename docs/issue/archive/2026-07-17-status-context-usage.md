...
discard_reason:
pending_reason:
close_reason: ["done:実装済みを確認 (v0.68.2 時点でランド済み、issue close 漏れ)。protocol の SessionContextUsage + snapshot.context、StatusPanel で formatContextUsage 表示 (上限は推定注記付き)","done:サイドバーバッジは採用せず Status タブ表示のみ (判断記録)","pending:/clear・compaction 跨ぎの実データ確認は未実施、問題が観測されたら再起票"]
blocked_by:
origin: 自リポ TODO
---
