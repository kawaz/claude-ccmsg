...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-22T16:53:40+09:00
discard_reason:
pending_reason:
close_reason: ["webui microbatch aggregation layer implemented (commit 057635da, pending next release push)","20ms window batches paragraphs across thinking boundaries into a single translate op","input-order contract restores original position","per-item failure falls back to source text for that paragraph only","cached items excluded from batch","all 4 acceptance criteria covered by unit tests (translate.test.ts 24 pass)","other 4 items in original issue confirmed already implemented as of 2026-07-21 body narrowing","done"]
blocked_by:
origin: 自リポ TODO
---
...
- [x] 表示中の翻訳対象段落を、複数 thinking 跨ぎで 1 回の translate op に束ねて送信する
- [x] 結果を元の thinking / 段落位置に正しく復元する
- [x] 部分失敗時は該当段落のみ原文 fallback する
- [x] `hostTextCache` と共存する (キャッシュ済み段落は batch に含めない)
