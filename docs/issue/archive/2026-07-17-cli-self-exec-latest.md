...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-21T03:38:04+09:00
discard_reason:
pending_reason:
close_reason: ["implemented: bin/ccmsgにsemverベースのself-execレール実装済み(plugin cacheパスから自バージョン抽出+PATH上ccmsgとのsemver比較で厳密に新しい時のみexec、単調増加保証で無限ループしない設計)","done: CCMSG_NO_SELF_EXEC=1を全テストハーネスに投入済み、bin/ccmsg.test.tsでテスト済み"]
blocked_by:
origin: 自リポ TODO
---
