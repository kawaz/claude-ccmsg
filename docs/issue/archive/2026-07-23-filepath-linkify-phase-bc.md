...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-23T12:07:40+09:00
discard_reason:
pending_reason:
close_reason: ["起票直後に worker が Phase B/C まで完走して整合した完成状態で納品、同セッションで実装完了 (commit 279dfe35, v0.73.0 予定)","fs_stat_batch op 実装 (認可は fsResolveForServe 3経路集約、存在有無を漏らさないnull統合、user-role only)","client 側 microtask batch キャッシュ実装 (disconnect invalidate)","monorepo 1672 pass","未裁定3点: batch粒度=メッセージ単位+microtask debounce, invalidation=peers縮小検出でsid clear, fold内先読み=ヒューリスティック排除で許容(厳密lazy化は必要が出たら別issue)","done"]
blocked_by:
origin: 自リポ TODO
---
