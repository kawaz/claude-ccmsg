...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-21T03:44:30+09:00
discard_reason:
pending_reason:
close_reason: ["done: session_status fold に teammates を追加し spawn/最終送信/最終受信+状態(spawned/active/idle/stopped)を集約","done: StatusPanel の TeamsSection/TeammateRow で色ドット+日本語ラベル+相対時刻表示を実装","done: 観測ベース推定である制約をコメント/状態ラベル定義に反映","done: session-status.test.ts でテスト済み"]
blocked_by:
origin: 自リポ TODO
---
