...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-21T03:26:29+09:00
discard_reason:
pending_reason:
close_reason: ["done: server.ts orderedMsgFrame() が subscribe wire を type,r,mid,from,seq[,to,reply_to],msg|msg_via,reply_via,ts 順に統一済み。当初案の msg 最後尾は msg→reply_via→ts に代替 (msg_via の predictive redirect 導入で更新)。上位ゴール (msg の silent 消失防止) は達成"]
blocked_by:
origin: 自リポ TODO
---
