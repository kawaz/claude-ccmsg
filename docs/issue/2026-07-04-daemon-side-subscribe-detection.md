...
## 実例: 孤児 subscribe が 2 日間 SESSIONS idle に表示され続けた (2026-07-23)

7/21 に終了した別リポセッション (業務リポ、ws サーバ関連) の subscribe (v0.71.3、bun プロセス) が親 Claude 終了後も launchd 配下の孤児として WS 接続を維持し、webui の SESSIONS idle に居座った。kawaz は webui から終了できず (peers に PID 情報が無い・切断手段も無い)、手動 kill で解消。daemon 側の親プロセス死活検出に加え、(a) peers への PID/親プロセス情報の追加、(b) webui からの強制切断ボタン、も対応候補。
