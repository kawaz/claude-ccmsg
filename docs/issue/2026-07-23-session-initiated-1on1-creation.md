...
- [ ] SKILL.md / ccmsg CLI の案内文に「kawaz への報告は post で room を作らず
  通常応答 (TL) で行う。1on1 が無い場合に create-room で自作しない」ガイドを追記
- [ ] reply_via_tl 拒否メッセージ等のレールで自作 1on1 への誘導を防ぐ検討
  (= 拒否時に「TL で答えて」と示唆する文言に倒せないか)
