...
- クライアントは自分の version と daemon の version を比較し、**client の方が厳密に新しい場合のみ** `{op:"shutdown", reason:"upgrade"}` を送って graceful stop させ、ensure 手順で新 version を spawn する (クライアント主導、newer-wins)。daemon が同じか新しければ何もしない。比較は `@ccmsg/protocol` の `compareVersions`
  - newer-wins なのは、gradual rollout で新旧 client が同時に daemon へ接触する状況で、互いに相手を降格させ合うフラッピングを起こさないため
...
#### 最新であるべきは daemon プロセスだけ

- **`subscribe` は daemon の更新をまたいで生き続ける**。daemon が新しくなっても subscribe プロセスは再起動しないし、更新されたことをセッションに通知もしない。subscribe の責務は「通知をセッションに出す」ことだけで、自分自身の version は責務ではない。再起動も更新案内も、そのたびにセッションのコンテキストを浪費する
- subscribe プロセスは起動時の plugin cache (`$CLAUDE_CONFIG_DIR/plugins/cache/ccmsg/ccmsg/<version>/`) から bun がソースを実行するが、**起動後はその cache ディレクトリに依存しない** (CLI 経路に動的 import は無く、必要なコードは起動時にオンメモリ。稼働中の subscribe が開いている cache 内ファイルは lsof で 0 件、実測)。旧 version のディレクトリが消えても走行中の subscribe は影響を受けない
- したがって **subscribe が使う経路 (hello / subscribe / イベント配信 / 再接続) を変えるときは、旧 version の subscribe が通らなくなることを意識的に決める**。壊れるかどうかを先に確認し、壊すなら DR に理由を書く。壊さないために互換分岐を足すのではない
- **「旧 subscribe が黙って再接続を続ける」のは仕様であって不具合ではない**。再起動しないこと自体を不具合と見て「直さねば」と動かない (fail-fast も自動再起動も、上のコンテキスト浪費に戻る)
- **後方互換は原則実装しない** (host 単位で丸ごと進化するイメージ。ccmsg は kawaz の 1 ホストに閉じた系で、新旧が併存する配布先を持たない)。互換のために形を残す・分岐を足すのは禁止で、必要に見えた時は**実装せず kawaz に相談する** [kawaz]
- **互換を切る場合は意図的な判断として DR に理由を書く**。切った時点で旧 subscribe は再接続しても通らなくなるので、**セッション側での張り直しが必要になる**ことも併せて記録する (例: `request_id` 必須化、DR-0029)

### 5. Crash 回復 [提案]

- 監視プロセスは置かない。crash → 次のクライアント接触 (ターン毎 hook 含む) で再 spawn される = 自然回復
- crash loop 対策はクライアント側: spawn 後の接続確立を backoff 付きでリトライし (`SPAWN_RETRY_DELAYS_MS`、25ms から 3s まで 12 回、累計約 10 秒。daemon は通常数百 ms で応答するので前半を細かく刻み、後半は起動が遅い環境の保険)、
  全て失敗したら `throw` してエラーを呼び出し元に伝播させる。hook 側の warn-once + 沈黙のような専用状態管理は置かない
  (エラーは hook の `try/catch` を通じて自然に表面化するため、追加の通知ロジックを持つ複雑さに見合う価値がない。daemon 起動失敗時に best-effort の hook が理由を出せないだけで実害は小さい)
...
- `daemon.log`: 10 MB (`ROTATE_BYTES`) を超えたら 1 世代ローテ (`daemon.log.1` に rename)。個人スケールの daemon log は数日〜数週で 10 MB に達しない量で、調査時に直前の 1 世代が読めれば足りる。`ccmsg daemon run --foreground` でデバッグ起動可能
...
- **version 不一致を等値比較で判定**: 不採用。gradual rollout で新旧 client が同時に接触するとフラッピングする (`docs/issue/2026-07-10-daemon-version-flapping-on-gradual-rollout.md`)。newer-wins (§4)
- **daemon 更新時に subscribe を自動再起動 / 張り直し案内を出す**: 不採用。通知と再接続がセッションのコンテキストを毎回浪費する (§4)
- **spawn 失敗を「5 回連続失敗で 1 回だけ警告し以降沈黙」のセッション単位状態で扱う**: 不採用。retry → throw だけで完結し、hook の try/catch で自然に表面化する (§5)
