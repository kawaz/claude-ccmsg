...
- 対案 (JS で culori 等により 12 段を計算して `:root` に流し込む) は、ガマット制約の削り方を自由に書ける利点があるが、app.css が生成物の列挙になり「CSS を読めばテーマが分かる」性質を失う。ガマット制約は §4.3 の C 係数表で十分近似できるため採らない。ブラウザの sRGB ガマットマッピングで濁る色相が実測で出た場合に限り、その色相の C 係数を落として対処する
...
- 1〜5 は背景同士なので差を小さく、5→6・8→9 で飛ぶ、11 / 12 は 1〜5 の上で文字コントラストを満たす L に置く (12 と 1〜3 の L 差 ≥ 0.7、11 と 1〜3 の差 ≥ 0.45)
...
- プリセット: 入力 8 個の組を名前付き JSON で複数持ち、切替で見比べる。既定プリセットは現行配色を再現したもの 1 つから始める
- 保存先は 2 段 (kawaz 裁定 r280、CS-Q1 = a+b): 編集中の値と最後に選んだプリセットは localStorage (端末ごと、即時)、名前を付けて保存したプリセットは daemon config (全端末共通、設定 op を 1 つ追加)。リポ同梱の既定プリセットは app 側の JSON
