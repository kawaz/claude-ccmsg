...
新規 config スキーマ (yaml or json、既存 daemon config と同居)。**裁定待ち [LN-Q1](../QUESTIONS.md#LN-Q1)**:
...
- **変数展開**: `{cwd}` / `{model}` / `{effort}` / `{prompt}` の 4 種、単純な文字列置換 (**裁定待ち [LN-Q2](../QUESTIONS.md#LN-Q2)**: 単純置換で shell escape 責務は kawaz 持ち、それとも daemon が引数リストとして安全に組み立てるか)
- `${...}` / `$(...)` は **shell が展開する部分** (template 内の bash 構文はそのまま bash に投げる、daemon は置換しない)
...
- **`dir_tree` op** — cwd ツリー描画用 (read-only、既存 `fs_list` の subset で depth 2 + ドット除外を daemon 側で処理)
  - request: `{op: "dir_tree", roots: string[], depth?: number, filter?: string}` (roots は config の root_dirs に一致するか検証)
...
- `session_launch`:
  1. template に 4 変数を str replace
  2. `Bun.spawn(["sh", "-c", cmd])` で shell 経由起動 (template が bash 展開を含むため)
...
`docs/QUESTIONS.md` に集約: **LN-Q1** (config ファイル形式・位置)、**LN-Q2** (変数展開の安全性)、**LN-Q3** (dir_tree の depth default とツリー表示のインタラクション)、**LN-Q4** (webui からの config re-read = daemon 再起動 vs signal reload)。
