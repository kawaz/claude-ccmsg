...
- room ごとの値が **有限の非負 number でなければその room の since_seq を無視**
  (= 未指定扱い → §2.3 の優先順位で次の cursor へ: since (mid) があればそれ、
  無ければ join snapshot。安全側 = 重複配信側に倒す)。負値・NaN・Infinity・
  非 number (JSON 経由でも生 socket 書き込みで到達しうる) が対象
