...
> **Superseded by [DR-0006](./DR-0006-id-scheme-v2.md)**: 整数 `uid` (`uid: 0` = User 予約) は型付き文字列 `id` (`u1`/`a1`...) に置き換え。`from`/`to` も同様。

### 3. Identity: hello 申告 + daemon 刻印

- 接続直後に `{op:"hello", role:"session", sid, repo, ws, cwd}` または `{op:"hello", role:"user"}` を送る
- **CLI の sid 自動判定順** [kawaz 2026-07-12 裁定、mid=9]: `--as-session <sid>` > `CCMSG_SID` > `CLAUDE_CODE_SESSION_ID` > (無ければ null)。旧設計にあった `CLAUDE_SESSION_ID` env は Claude Code 実環境に存在せず、実質デッドコードだったため削除
- **write 系 op (`post` / `create_room` / `next_room` / `leave` / `notify`) は identity 無しなら CLI が error 終了**する [kawaz 2026-07-12、docs/issue/2026-07-12-prevent-u1-masquerade-on-missing-sid.md]。以前は sid 未取得の CLI が `role: "user"` へ暗黙 fallback して u1 (User) 名義で post し、受信側の「`from: "u1"` = ユーザ発言」判定を狂わせる事故 (room r2 mid2/mid4) が発生した。u1 発行経路は webui backend の `role: "user"` hello 一本に絞る
- **subscribe だけは identity 無しでも許容**し、CLI は stderr に警告を出しつつ `role: "user"` で hello する。webui 未実装期に kawaz が素のターミナルから u1 として観測する経路の温存
- daemon 側は変更なし: `IDENTITY_OPS` (post/create_room/next_room/…/subscribe/notify) が hello 必須ゲート、`role: "user"` hello は webui backend の合法経路として引き続き受理する。CLI 経由の u1 化を塞ぐ責任は CLI 側にある (指示側 client のポリシーであって、wire protocol の禁則ではない)
...
- **write 系 identity 無しを warning にとどめて post を通す (旧挙動)**: 不採用 [kawaz 2026-07-12、mid=9]。stderr 警告では Monitor 経由の AI サイドカー・agent は気付けず、事実として r2 で誤配信が起きた。hard error にして「気付かせる」ほうが安全
