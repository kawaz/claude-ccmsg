...
### 7. 追補 (2026-07-11): containment root を「検証済み repo container (repo_root)」へ拡張 [kawaz]

kawaz 裁定「どうリポに ws/wt 複数あるならソレもみれるようにして欲しい」を受け、fs_list/fs_read の containment root を条件付きで cwd からリポ container に広げる:

- **導出 (hook)**: `bump-semver vcs get` の facts で **backend が jj かつ workspace 名が非空のときのみ** `dirname(workspace root)` を repo_root として session state ファイルに書く。kawaz の jj 運用 (`<repo>/<ws>` ネスト) では dirname = 全 workspace の親に一致する。**git linked worktree は未対応** (既知の制約): kawaz の git worktree は owner ディレクトリ直下に他リポと同列に並ぶため、dirname では「リポの container」でなく「owner の全リポ」に広がってしまう。git worktree の正しい container 特定には `git worktree list` での裏取りが必要で、必要になった時点で拡張する
