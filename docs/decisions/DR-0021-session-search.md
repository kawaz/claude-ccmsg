...
## 3. 設計判断 (裁定待ち SS-Q1..2 は QUESTIONS.md)

### 3.1 daemon 側: 新 op 群

- `session_search`: § 2.1 のパラメータ → § 2.2 の 3 段 → § 2.3 の行データを返す
- 過去セッションの transcript / files 閲覧: 現行 transcript_read / fs_list / fs_read は
  **hello 済み sid** に紐づく。未接続セッション向けに「daemon が jsonl から cwd を復元して
  仮想 session 扱いで読ませる」拡張が必要。認可は user role のみ (webui 経由限定)
- 走査範囲は daemon が既に agents poll で検出している config_dir 群の projects/ 配下に限定

### 3.2 pinned list の保存先

localStorage (webui) を想定 — daemon 側の永続化は不要 (kawaz の「pined SESSION リスト的なの」
は閲覧マーキングであり、共有状態ではないと解釈)。
