...
- **ST-Q2**: 表示場所は? (mid=25 でタブ案 → mid=26 で TL 下余白案も浮上)
  - (a) Status タブ (Files/Timeline/Rooms の並びに追加)
  - (b) Timeline 下部の余白領域に常駐コンパクトパネル
  - (c) 両方 — TL 下に要約 1-2 行 (走行中 wf / in_progress TODO のみ)、詳細は Status タブ — **AI 推し**: 一覧性 (常駐) と詳細 (タブ) の役割が異なり、要約行はコンパクトに収まる
  - 詳細: `docs/decisions/DR-0020-session-status-tab.md#21`
