...
C9 b/c と C11 a の 404 は v0.76.1 で修正済み (ルート相対の基準を cwd に修正 + 行指定時は code モード強制)。リロード後に該当リンクを踏み直してください。
...
- [ ] a: コンテキスト溢れ等で停止したセッションがサイドバー最上段の Error セクションに danger 色で出る (busy より優先)
- [ ] b: 行内にエラー理由 (Prompt is too long 等) が出る
- [ ] c: セッションが復帰する (本物の assistant 応答が出る) と Error から消える
- [ ] d: TL で該当行がエージェント発話でなく danger 色の通知行になっている
