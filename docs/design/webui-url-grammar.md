...
- ペイン幅、fold の開閉、サイドバーの在り / 無し (どのフォームが開いている
  かは `sb.panel` が持つが、それを収めるサイドバーが出ているかは持たない —
  画面共有のために消した状態はその窓限りのもので、貼って共有する意味がない)。
  サイドバーと本文が横に並びきらない幅では消す操作自体が無く、代わりに
  「今どちらを見ているか」になるが、これも同じ理由で URL には載せない
  (開いた直後にどちらを見せるかは `path` が対象を名指しているかで決まるので、
  リンクを貼れば相手も同じ側から始まる)
