...
describe("createOriginsFile", () => {
  ...
  test("missing file reads as an empty set", async () => {
    const of = createOriginsFile(tmpFile(), silentLog);
    expect((await of.get()).size).toBe(0);
  });

  // Round-trip of the format `ccmsg origins add` writes (JSON string[]).
  test("reads a JSON string[] as a set of origins", async () => {
    const file = tmpFile();
    fs.writeFileSync(file, JSON.stringify(["https://a.example", "https://b.example"]));
    const of = createOriginsFile(file, silentLog);
    expect((await of.get()).has("https://a.example")).toBe(true);
    expect((await of.get()).has("https://b.example")).toBe(true);
    expect((await of.get()).size).toBe(2);
  });
  ...
  test("picks up additions on the next get() after the file changes", async () => {
    const file = tmpFile();
    fs.writeFileSync(file, JSON.stringify(["https://a.example"]));
    const of = createOriginsFile(file, silentLog);
    expect((await of.get()).has("https://new.example")).toBe(false);
    fs.writeFileSync(file, JSON.stringify(["https://a.example", "https://new.example"]));
    fs.utimesSync(file, new Date(), new Date(Date.now() + 5000));
    expect((await of.get()).has("https://new.example")).toBe(true);
  });
  ...
  test("file removal revokes previously cached origins", async () => {
    const file = tmpFile();
    fs.writeFileSync(file, JSON.stringify(["https://a.example"]));
    const of = createOriginsFile(file, silentLog);
    expect((await of.get()).has("https://a.example")).toBe(true);
  ...
  test("corrupted JSON reads as empty and warns once per mtime", async () => {
    const file = tmpFile();
  ...
  test("non-string array entries are ignored", async () => {
    const file = tmpFile();
    fs.writeFileSync(file, JSON.stringify(["https://a.example", 42, null]));
    const of = createOriginsFile(file, silentLog);
    expect((await of.get()).size).toBe(1);
    expect((await of.get()).has("https://a.example")).toBe(true);
  });
});
