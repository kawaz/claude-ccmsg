 ...
 * 0.1 は kawaz 裁定 (r135m22): 想定ケースは「英文中心 + kawaz の言葉が一言
 * 引用される」形で、実例の比率は 0.06。逆側の日本語技術文は英単語の方が
 * 1 語あたりの文字数が多い性質を踏まえても、骨格の かな が効いて 0.1 を
 * 上回る (識別子 4 連の実例で 0.19)。識別子が極端に支配的な日本語 1 文は
 * なお誤訳側に落ち得るが、その場合も原文タブは無傷で残る。 */
const JAPANESE_RATIO_THRESHOLD = 0.1;
