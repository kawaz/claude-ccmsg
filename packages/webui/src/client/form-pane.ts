 ...
 * 下限・上限も #sidebar と同じ扱い = 潰れる手前 (PANE_MIN_PX) まで動かせる。
 * `availablePx` はこのペインの左辺から右端までの幅で、ドラッグ時のみ渡す。 */
export function clampFormPaneWidth(w: number, availablePx?: number): number {
  return clampPanePx(w, FORM_PANE_DEFAULT_PX, availablePx);
}
